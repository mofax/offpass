...
interface PasswordManagerState {
  ...
  addCredential: (credential: Omit<Credential, "id" | "createdAt" | "lastModified">) => Promise<Credential>;
  updateCredential: (id: string, updates: Partial<Omit<Credential, "id" | "createdAt">>) => Promise<Credential>;
  deleteCredential: (id: string) => Promise<void>;
  
  // Settings management
  updateSettings: (newSettings: Partial<AppSettings>) => Promise<void>;
  updateVaultSettings: (settings: Partial<VaultData["settings"]>) => Promise<void>;
...
const PasswordManagerContext = createContext<PasswordManagerState | undefined>(undefined);
...
export function PasswordManagerProvider({ children }: { children: ReactNode }) {
  ...
  async function renameVault(id: string, newName: string) {
    ...
    try {
      if (!masterPassword || !vaultData || !currentVault || currentVault.id !== id) {
        throw new Error("Vault not open or mismatch");
      }
      
      const updatedVault = await updateVault(id, masterPassword, vaultData, newName);
  ...
  async function changeMasterPassword(currentPassword: string, newPassword: string) {
    ...
    try {
      ...
      await updateVault(currentVault.id, currentPassword, vaultData!, undefined);
  ...
  async function addCredential(credential: Omit<Credential, "id" | "createdAt" | "lastModified">) {
    setIsLoading(true);
    setError(null);
  ...
  async function deleteCredential(id: string) {
    ...
    try {
      ...
      await deleteCredentialFromVault(currentVault.id, masterPassword, id);
  ...
  async function updateVaultSettings(newSettings: Partial<VaultData["settings"]>) {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      await updateSettingsInVault(currentVault.id, masterPassword, updatedSettings);
