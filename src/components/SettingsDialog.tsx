...
export default function SettingsDialog({ isOpen, onClose }: SettingsDialogProps) {
  ...
  const handleChangeMasterPassword = async () => {
    ...
    try {
      ...
      setPasswordChangeError("Failed to change password. Current password may be incorrect.");
    }
  };
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <Tabs defaultValue="appearance" className="mt-4">
          ...
          <TabsContent value="security" className="space-y-4 py-4">
            <div>
              <h3 className="text-lg font-medium mb-2">Auto-Lock Timeout</h3>
              <p className="text-sm text-muted-foreground mb-2">
                Set how long until the vault automatically locks after inactivity
              </p>
              <div className="flex items-center space-x-2">
                <Button
                  variant={settings.autoLockTimeout === 5 ? "default" : "outline"}
                  onClick={() => handleUpdateAutoLockTimeout(5)}
                >
                  5 min
                </Button>
                <Button
                  variant={settings.autoLockTimeout === 15 ? "default" : "outline"}
                  onClick={() => handleUpdateAutoLockTimeout(15)}
                >
                  15 min
                </Button>
                <Button
                  variant={settings.autoLockTimeout === 30 ? "default" : "outline"}
                  onClick={() => handleUpdateAutoLockTimeout(30)}
                >
                  30 min
                </Button>
                <Button
                  variant={settings.autoLockTimeout === 60 ? "default" : "outline"}
                  onClick={() => handleUpdateAutoLockTimeout(60)}
                >
                  1 hour
                </Button>
              </div>
            ...
            <div className="pt-4 border-t">
              ...
              <div className="space-y-2">
                ...
                {passwordChangeError && (
                  <div className="text-destructive text-sm">{passwordChangeError}</div>
                )}
