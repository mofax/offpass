...
export default function CredentialList({
  ...
  const filteredCredentials = vaultData.credentials.filter((credential) => {
    const matchesSearch = searchTerm === "" || 
      ...
      (credential.url && credential.url.toLowerCase().includes(searchTerm.toLowerCase()));
    
    const matchesCategory = selectedCategory === null || credential.category === selectedCategory;
    ...
    <div className="p-4 flex-1 overflow-auto">
      ...
      {filteredCredentials.length === 0 ? (
        <div className="text-muted-foreground text-center p-8">
          {searchTerm ? "No matching credentials found." : "No credentials in this category."}
        </div>
      ) : (
              ...
              <CardContent className="p-0">
                <div className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleDelete(credential.id, credential.title)}
                        title="Delete"
                      >
                  ...
                  <div className="mt-2 text-xs text-muted-foreground">
                    Last modified: {formatDate(credential.lastModified)}
                  </div>
                </div>
              </CardContent>
            </Card>
